...
def setup_ranger_admin(upgrade_type=None):
  ...
  Execute(('ln','-sf', format('{ranger_home}/conf'), format('{ranger_home}/ews/webapp/WEB-INF/classes/conf') ),
    not_if=format("ls {ranger_home}/ews/webapp/WEB-INF/classes/conf"),
  ...
  if os.path.isfile(params.ranger_admin_default_file):
    ...
    Logger.warning('Required file {0} does not exist, copying the file to {1} path'.format(
  ...
  Link(format('{ranger_home}/ews/ranger-admin-start'),
       to=format('{ranger_home}/ews/start-ranger-admin.sh'))

  Link(format('{ranger_home}/ews/ranger-admin-stop'),
       to=format('{ranger_home}/ews/stop-ranger-admin.sh'))
...
def do_keystore_setup(upgrade_type=None):
  ...
  if not is_empty(params.ranger_credential_provider_path) and (params.ranger_audit_source_type).lower() == 'db' and not is_empty(
...
def setup_usersync(upgrade_type=None):
  ...
  Execute(('mkdir','-p', '/etc/ranger/usersync'),
    sudo=True)

  Execute(('ln','-sf', format('{stack_root}/current/ranger-usersync/conf'), format('{ranger_ugsync_conf_etc}')),
    sudo=True)
  ...
  Link(format('{usersync_home}/ranger-usersync-start'),
       to=format('{usersync_home}/start.sh'))

  Link(format('{usersync_home}/ranger-usersync-stop'),
       to=format('{usersync_home}/stop.sh'))
